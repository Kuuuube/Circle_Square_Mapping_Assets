...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = ((sgnxy / y) * _sqrt((-1 + _sqrt(1 + 4 * x4 * y2 + 4 * x2 * y4)) / (2 * (x2 + y2))))
        v = ((sgnxy / x) * _sqrt((-1 + _sqrt(1 + 4 * x4 * y2 + 4 * x2 * y4)) / (2 * (x2 + y2))))
        return u, v
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv / v) * _sqrt((1 - _sqrt(1 - 4 * u4 * v2 - 4 * u2 * v4)) / (2 * (u2 + v2))))
        if abs(u) > _epsilon:
            y = ((sgnuv / u) * _sqrt((1 - _sqrt(1 - 4 * u4 * v2 - 4 * u2 * v4)) / (2 * (u2 + v2))))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v

...
def _elliptical_square_to_disc(x, y):
    ...
    B = 0.99
    try:
        x = (x * math.sqrt((y2 * (B + 1 - B * y2) - (B + 1 - B * x2) * (B + 1 - B * y2)) / (x2 * y2 - (B + 1 - B * x2) * (B + 1 - B * y2))))
        y = (y * math.sqrt((x2 * (B + 1 - B * x2) - (B + 1 - B * x2) * (B + 1 - B * y2)) / (x2 * y2 - (B + 1 - B * x2) * (B + 1 - B * y2))))
        return x, y
...
def _elliptical_disc_to_square(u, v):
    ...
    B = 0.99
    try:
        x = ((sgnu / math.sqrt(2 * B)) * math.sqrt(B + 1 + B * u2 - v2 - math.sqrt(pow((B + 1 + B * u2 - v2), 2) - 4 * B * (B + 1) * u2)))
        y = ((sgnv / math.sqrt(2 * B)) * math.sqrt(B + 1 - u2 + B * v2 - math.sqrt(pow((B + 1 - u2 + B * v2), 2) - 4 * B * (B + 1) * v2)))
        return x, y
    except ValueError:  # sqrt of a negative number
        return None

...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = (x * _sqrt((x2 + y2 - 2 * x2 * y2) / ((x2 + y2) * (1 - x2 * y2))))
        v = (y * _sqrt((x2 + y2 - 2 * x2 * y2) / ((x2 + y2) * (1 - x2 * y2))))
        return u, v
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv / v) * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2 * (2 - u2 - v2)))) / (2 * (2 - u2 - v2))))
        if abs(u) > _epsilon:
            y = ((sgnuv / u) * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2 * (2 - u2 - v2)))) / (2 * (2 - u2 - v2))))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v

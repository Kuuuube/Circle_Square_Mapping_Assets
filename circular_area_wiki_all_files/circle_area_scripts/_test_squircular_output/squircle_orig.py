...
def _elliptical_square_to_disc(x, y):
    n = 2
    try:
        return x * pow(math.sqrt(1.0 - y * y / 2.0),(n/2)) , y * pow(math.sqrt(1.0 - x * x / 2.0),(n/2))
...
def _elliptical_disc_to_square(u, v):
    ...
    if r2 > 1:  # we're outside the disc
        return
    try:
        x = (math.sqrt(2 + 2 * u * n + math.sqrt(2) + pow(u*n, 2) - pow(v*n, 2)) - math.sqrt(2 - 2 * u * n + math.sqrt(2) + pow(u*n, 2) - pow(v*n, 2)))/(2*n)
        y = (math.sqrt(2 + 2 * v * n + math.sqrt(2) + pow(u*n, 2) - pow(v*n, 2)) - math.sqrt(2 - 2 * v * n + math.sqrt(2) + pow(u*n, 2) - pow(v*n, 2)))/(2*n)
        return x, y
    except ValueError:  # sqrt of a negative number
        return None

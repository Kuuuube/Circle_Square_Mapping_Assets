...
def _elliptical_square_to_disc(x, y):
    ...
    B = 0.01
    try:
        x = (x * math.sqrt(1 - B * y2))
        y = (y * math.sqrt(1 - (1 - B) * x2))
        return x, y
...
def _elliptical_disc_to_square(u, v):
    ...
    B = 0.01
    try:
        x = ((sgnu / (math.sqrt(2 * (1 - B)))) * math.sqrt(1 + (1 - B) * u2 - B * v2 - math.sqrt(pow((1 + (1 - B) * u2 - B * v2), 2) - 4 * (1 - B) * u2)))
        y = ((sgnv / (math.sqrt(2 * B))) * math.sqrt(1 - (1 - B) * u2 + B * v2 - math.sqrt(pow((1 - (1 - B) * u2 + B * v2), 2) - 4 * B * v2)))
        return x, y
    except ValueError:  # sqrt of a negative number
        return None

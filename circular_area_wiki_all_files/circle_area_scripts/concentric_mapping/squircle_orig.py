...
def _stretch_square_to_disc(x, y):
    ...
    # if abs(x2) > abs(y2):
    if absx >= absy:
        x = (x * math.cos((math.pi / 4) * (y / x)))
        y = (x * math.sin((math.pi / 4) * (y / x)))
    else:
        x = (y * math.sin((math.pi / 4) * (x / y)))
        y = (y * math.cos((math.pi / 4) * (x / y)))
...
def _stretch_disc_to_square(u, v):
    ...
    if u2 > v2:
        x = (sgnu * math.sqrt(u2 + v2) * 1)
        y = (sgnu * math.sqrt(u2 + v2) * ((4 / math.pi) * math.atan(v / u)))
    else:
        x = (sgnv * math.sqrt(u2 + v2) * ((4 / math.pi) * math.atan(u / v)))
        y = (sgnv * math.sqrt(u2 + v2) * 1)
    return x, y

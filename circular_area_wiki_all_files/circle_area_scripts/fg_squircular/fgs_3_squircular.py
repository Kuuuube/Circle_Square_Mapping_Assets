...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = ((x * _sqrt(x2 + y2 - x2 * y2)) / (_sqrt(x2 + y2)))
        v = ((y * _sqrt(x2 + y2 - x2 * y2)) / (_sqrt(x2 + y2)))
        return u, v
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        x = (sgnuv / vsqrttwo * _sqrt(u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2))))
        y = (sgnuv / usqrttwo * _sqrt(u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2))))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v

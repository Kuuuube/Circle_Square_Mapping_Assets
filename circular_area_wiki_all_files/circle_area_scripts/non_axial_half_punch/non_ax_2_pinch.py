...
def _fgs_square_to_disc(x, y):
    ...
    u = (x / (pow((x2 + y2), 0.25) * pow((1 + x2 + y2), 0.25)))
    v = (y / (pow((x2 + y2), 0.25) * pow((1 + x2 + y2), 0.25)))
    return u, v
...
def _fgs_disc_to_square(u, v):
    ...
    cuberoottwo = pow(2, 0.25)
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv / u) * _sqrt((1 - _sqrt(1 - 4 * u2 * v2 * pow((u2 + v2), 2))) / (2 * (u2 + v2))))
        if abs(u) > _epsilon:
            y = ((sgnuv / v) * _sqrt((1 - _sqrt(1 - 4 * u2 * v2 * pow((u2 + v2), 2))) / (2 * (u2 + v2))))
        return x, y
...
def _elliptical_disc_to_square(u, v):
    ...
    try:
        x = (x / (pow((x2 + y2), 0.25) * pow((1 + x2 + y2), 0.25)))
        y = (y / (pow((x2 + y2), 0.25) * pow((1 + x2 + y2), 0.25)))
        return x, y
    except ValueError:  # sqrt of a negative number
        return None

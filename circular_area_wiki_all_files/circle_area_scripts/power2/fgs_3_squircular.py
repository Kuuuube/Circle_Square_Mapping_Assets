...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = ((_sqrt((x2 + y2 - 2 * B * x2 * y2) / ((x2 + y2) * (1 + x2 * y2 - 2 * B * x2 * y2)))) * x)
        v = ((_sqrt((x2 + y2 - 2 * B * x2 * y2) / ((x2 + y2) * (1 + x2 * y2 - 2 * B * x2 * y2)))) * y)
        return u, v
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2 * (u2 + v2 + 2 * B * (1 - u2 - v2))))) / (2 * (u2 + v2 + 2 * B * (1 - u2 - v2)))))*(1/v))
        if abs(u) > _epsilon:
            y = ((sgnuv * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 4 * u2 * v2 * (u2 + v2 + 2 * B * (1 - u2 - v2))))) / (2 * (u2 + v2 + 2 * B * (1 - u2 - v2)))))*(1/u))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v

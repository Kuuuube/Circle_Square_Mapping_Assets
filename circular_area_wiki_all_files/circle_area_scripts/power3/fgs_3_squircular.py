...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = (((sgnxy / (_sqrt(x2 + y2))) * (_sqrt((1 - _sqrt(1 - 2 * (3 * B - 2) * x4 * y2 - 2 * (3 * B - 2) * x2 * y4 + 3 * B * (3 * B - 2) * x4 * y4)) / (3 * B - 2)))) * (1 / y))
        v = (((sgnxy / (_sqrt(x2 + y2))) * (_sqrt((1 - _sqrt(1 - 2 * (3 * B - 2) * x4 * y2 - 2 * (3 * B - 2) * x2 * y4 + 3 * B * (3 * B - 2) * x4 * y4)) / (3 * B - 2)))) * (1 / x))
        return u, v
    except ZeroDivisionError:  # sqrt of a negative number
        return x, y
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 2 * u2 * v2 * (3 * B - (3 * B - 2) * u4 - 2 * (3 * B - 2) * u2 * v2 - (3 * B - 2) * v4)))) / (3 * B - (3 * B - 2) * u4 - 2 * (3 * B - 2) * u2 * v2 - (3 * B - 2) * v4))) * (1 / v))
        if abs(u) > _epsilon:
            y = ((sgnuv * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 2 * u2 * v2 * (3 * B - (3 * B - 2) * u4 - 2 * (3 * B - 2) * u2 * v2 - (3 * B - 2) * v4)))) / (3 * B - (3 * B - 2) * u4 - 2 * (3 * B - 2) * u2 * v2 - (3 * B - 2) * v4))) * (1 / u))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v

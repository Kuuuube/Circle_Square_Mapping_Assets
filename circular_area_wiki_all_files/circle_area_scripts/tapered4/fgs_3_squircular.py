...
def _fgs_square_to_disc(x, y):
    ...
    try:
        u = ((sgnxy / (y * _sqrt(x2 + y2))) * _sqrt(1 - _sqrt(1 - 2 * x4 * y2 - 2 * x2 * y4 + 3 * x4 * y4)))
        v = ((sgnxy / (x * _sqrt(x2 + y2))) * _sqrt(1 - _sqrt(1 - 2 * x4 * y2 - 2 * x2 * y4 + 3 * x4 * y4)))
        return u, v
    except ValueError:
        return None
...
def _fgs_disc_to_square(u, v):
    ...
    try:
        if abs(v) > _epsilon:
            x = ((sgnuv / v) * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 2 * u2 * v2 * (3 - u4 - 2 * u2 * v2 - v4)))) / (3 - u4 - 2 * u2 * v2 - v4)))
        if abs(u) > _epsilon:
            y = ((sgnuv / u) * _sqrt((u2 + v2 - _sqrt((u2 + v2) * (u2 + v2 - 2 * u2 * v2 * (3 - u4 - 2 * u2 * v2 - v4)))) / (3 - u4 - 2 * u2 * v2 - v4)))
        return x, y
    except ZeroDivisionError:  # sqrt of a negative number
        x = u
        y = v
